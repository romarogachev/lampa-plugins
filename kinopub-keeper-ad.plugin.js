 ...
 *  Version: 2.6.1-diag (лог событий Lampa)
 * ============================================================
 */
...
(function () {
    ...
    function initPlugin() {
        ...
        // Визуальный лог событий Lampa — диагностика рекламной паузы
        var _evLog = [];
        var _evEl = null;
        function _showEvLog() {
            if (!_evEl) {
                _evEl = document.createElement('div');
                _evEl.style.cssText = 'position:fixed;bottom:10px;left:10px;z-index:99999;background:rgba(0,0,0,0.9);color:#ff0;font-size:11px;padding:8px;max-width:800px;border-radius:6px;pointer-events:none;line-height:1.5;';
                document.body.appendChild(_evEl);
            }
            _evEl.innerHTML = '<b>Events:</b><br>' + _evLog.slice(-10).join('<br>');
        }
        try {
            var _origSend = Lampa.Listener.send.bind(Lampa.Listener);
            Lampa.Listener.send = function(name, data) {
                _evLog.push((Date.now() % 100000) + ': ' + name);
                setTimeout(_showEvLog, 50);
                return _origSend(name, data);
            };
        } catch(e) {}

        console.log('[KP Keeper] v2.6.1-diag инициализирован.');
    }
